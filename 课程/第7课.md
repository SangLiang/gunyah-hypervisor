...
把这条广播链从「写 `.ev`」到「运行时调 handler」整条画出来：
...
flowchart TB
    ...
    subgraph GEN["② 代码生成层 tools/events/"]
        PARSE["parser.py + ir.py<br/>解析 + 优先级/锁校验"]
        PICKLE["build/events.pickle"]
        TC["c.tmpl → trigger_*.c<br/>分发表实现"]
        TEH["handlers.h.tmpl → event_handlers.h"]
    end

    subgraph RUN["③ 运行时"]
        PUB["boot.c 等业务代码<br/>trigger_<event>_event()"]
...
读法：左到右是**构建期**（扫 `.ev` → 生成分发表），下到下是**运行期**（业务代码 `trigger_*` → 查表逐个调 handler）。`boot.c` 只出现在最左下角的「业务代码」框，它**既不读 `.ev`，也不认识任何订阅者**——订阅关系全在中间那层构建期就固化进 dispatch 函数了。这就是上表「`boot.c` 要不要 `#include` 各模块 = 不要」的图解。
...
| [ev流程图.md](../ev流程图.md) | 事件订阅发布机制的完整流程图集（含五种事件类型、对象链、guest 交互等后续课程用到的图） |
