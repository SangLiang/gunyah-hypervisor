...
上一课：线程对象造出来了；`block_bits` 是「不能跑」的原因位，全 0 才会进调度器。本课只解决一件事：**FPRR 怎样按每颗物理 CPU 一张就绪表选出下一个**。
...
`can_be_scheduled()` 看 `scheduler_block_bits` 是否全空（第 14 课）。全 0 才能入队；有任何一位，这条线程对 FPRR 就是隐形的，不进 runqueue。`unblock` 清掉**最后一位**时，才 `add_thread_to_scheduler`。
...
1. **FPRR 选的是线程，不是 VM。** VCPU 只是 kind 不同的线程。
