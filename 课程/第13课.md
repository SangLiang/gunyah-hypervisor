...
仍被第 11 课的启动锁锁着。RootVM 就算 poweron 了，也要等 `boot_cpu_start` 解锁、再进 idle yield，才会真正占住物理 CPU。
...
第 9 课造出的 root partition 当时只有一块启动堆；这里才把平台探测到的其余 RAM 登记进去。后面加载 package、建对象都从这块堆拿。

注意：这是 `partition_add_ram_range(..., partition_root)` **给 root 新登记无主 RAM**，不是从 `partition_hyp` 过户。hyp 只接管自己的镜像/私有堆；其余 RAM 从未先归 hyp。造 root 时 hyp 只 donate 过一小块启动堆（第 9 课 ④）。

`idle_thread_init`（第 10 课的「补全」）也在默认 0 这一档：对象齐了，**还没有**跳进 `idle_loop`。同档还有 hyp 自己的维护中断等，本课不展开；VGIC 注入是第 22–23 课。
...
| [第 9 课](第9课.md) | hyp 只认领镜像；其余 RAM 本课才登记给 root |
| [第 10 课](第10课.md) | 造 idle 对象 vs 本课跳进去 |
