...
上一课在 `boot_cold_init` 上已经建好 `partition_hyp`，镜像占用的物理页写进了 `memdb`，hyp 的 heap 可以 `alloc`。本课只解决一件事：**Boot CPU 的 idle 线程其实更早就建了，而且当时对象不完整**。不讲 FPRR，也不讲怎样真正切到 VCPU。
...
flowchart LR
  ...
  B --> C["boot_cold_init 起\npartition_hyp / memdb / cpu_cold / warm"]
