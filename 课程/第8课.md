...
上一课：`trigger_*` 是广播。本课只解决两件事：**订阅写在各模块自己的 `.ev` 里**；以及 **优先级数字怎样排出谁先谁后**。一页 RAM 怎样记进 `partition` / `memdb`，是第 9-12 课。
...
把 `boot_cold_init` 上本课用到的几档排出来（`partition` / `memdb` 怎么登记，第 9-12 课再走）：
...
3. **不要在本课走完一页 RAM。** 顺序表只为说明优先级；`partition` / `memdb` 的登记是第 9-12 课。
