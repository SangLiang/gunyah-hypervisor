...
「一页 RAM 归谁管」不是问哪根 CPU、哪个线程正在用这块内存，而是问：**这 4KB 物理格子，现在记在哪个账户名下。** 账本防的是 hyp 自己把页发错；guest 运行时摸不摸得到，靠的是后面的 Stage-2。
...
整机 RAM 按页切开（第 3 课）。启动时 hyp 镜像已经占了一截格子，后面还要从格子里抠出对象。没有账本，谁都可以拿走任意一页。本课就是先把镜像占着的那些物理页登记到 `partition_hyp` 名下。

两件不要当成同一件事：

| 问题 | 谁回答 |
|------|--------|
| 这个地址数字落到哪页物理 RAM？ | 页表 / MMU（第 3、6 课） |
| 这页物理 RAM 现在属于哪个账户？ | memdb + partition（本课） |
...
3. **启动路径里不要把 idle、不要把 Stage-2 扯进来。** 本课只到 `partition_hyp` / root 两个 partition，以及 memdb 里对应的记录。账本和 Stage-2 / MMU 的分工见下一节。也不要把 partition 理解成「一段已标注所有者的 PA」：账户下面可以挂很多段、而且可以不连续。

---

## 5. 容易混：不是软件 MMU，也不是 guest 的运行时铁门

**不是 MMU 的软件实现。** MMU 是硬件翻译机：页表把 VA（或后面 guest 的 IPA）译成 PA。memdb 是房产登记：这页 PA 写在谁名下。同一页可以同时有这两条记录——页表说某个虚地址对应这页；账本说这页归 `partition_hyp`。

**也不是 guest 每次访存走的权限检查。** guest 指令碰内存时，硬件查 Stage-2（第 18–21 课）。memdb 是 hyp **准备 insert / update / donate / map 时对自己的检查**：这页是不是这个账户的、角色对不对。对不上就拒绝，避免把 hyp 自己的页、或已经给别人的页，错误交给 guest。

**guest 不知道真实 PA，挡不住填错表。** Stage-2 只保证「表上怎么写，就怎么译」。hyp 若把 hyp 自己的 PA 写进 guest 的 Stage-2，guest 仍然只用 IPA、仍然不知道 PA 是多少，但已经在读写 hyp 的内存。MMU 不知道「这页是 hyp 的、不许给 guest」；知道这件事的是账本。

若见过 **pKVM**：主机把受保护 VM 的内存一点一点 pin 住、再从主机页表拆掉，也是在圈物理页专供那台 VM。本仓库没有 pKVM。同工的是 **一页不能两用**；异曲的是 pKVM 还要对抗 Linux 换页（所以要 pin），Gunyah 是 Type-1，开机时 RAM 就不在会换页的主机内核里，用 memdb 记归属即可。这里不展开 pKVM。

---

## 本课你要带走的三句话

1. `partition` **是内存账户（归属单位），不是一段 PA，也不是线程；**`memdb` **按物理页记当前 owner；**`allocator` **是账户里用来** `alloc` **的堆。** 账本不是软件 MMU，也不是 guest 访存时的铁门。
...
2. `memdb_insert` 和 `memdb_update` 各做哪一类事？bootmem 那段为什么要用 `update`？owner 是线程吗？memdb 是软件 MMU，还是 hyp 过户前的自检？
...
| [第 6 课](第6课.md)                            | 开 MMU 是填页表、打开翻译；和本课的产权登记不是一回事                 |
| [第 8 课](第8课.md)                            | 这几个 handler 的优先级从哪来                            |
| [第 13 课](第13课.md)                          | 其余平台 RAM 登记给 root                              |
| [第 18 课](第18课.md)                          | Stage-2 才是 guest 访存的铁门；四个角色对齐                    |
