...
4. **VCPU 专用** `vcpu_handle_object_create_thread`：再挂 **`SCHEDULER_BLOCK_VCPU_OFF`**。idle 则挂永久的 `SCHEDULER_BLOCK_IDLE`（第 10 课）。
...
| [第 10 课](第10课.md) | idle 的 create 挂永久 `BLOCK_IDLE` |
